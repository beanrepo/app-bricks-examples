...
The release notes are generated by `scripts/release_notes.py` from the diff with the previous release: the previous RC or stable for a pre-release, the previous stable for a stable release. They list the examples added, fixed and removed (an example is a directory holding an `app.yaml`), the commits in between and the full changelog link. Stable releases get a manual review of the description afterwards. `python3 scripts/release_notes.py --version <version>` previews them locally.
